...
class ViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        self.selfUserName = String(format: "%@", userID)
        
        let config = ZegoUIkitPrebuiltCallConfig()
        config.menuBarButtons = []
        ZegoUIKitPrebuiltCallInvitationService.shared.initWithAppID(yourAppID, appSign: yourAppSign, userID: selfUserID, userName: selfUserName!)
        ZegoUIKitPrebuiltCallInvitationService.shared.delegate = self
        
        
        let label: UILabel = UILabel()
        label.frame = CGRect.init(x: 100, y: 50, width: 200, height: 20)
        label.text = String(format: "YourUserID: %@", userID)
